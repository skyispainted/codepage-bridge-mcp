...
- `npm install`
- `npm run build`
- `claude mcp add --scope user codepage-bridge`
- 输出后续必须执行的 Claude Code 配置说明
...
> 注意：这里的“一键安装”是指**自动构建并自动配置 Claude Code MCP**。
>
> 但它仍然依赖用户本机已有 `node`、`npm` 和 `claude`。
>
> 如果你想让用户完全不需要本地 `npm install`，那就必须额外发布：
>
> - 预构建 npm 包；
> - 或平台二进制；
> - 或 GitHub Release 中的打包产物。

当前仓库还是源码发布模式，所以首次安装仍需要本地构建。
